...
for(var i = 0 ; i < shoppingCartProduct.length ; i++) {
  ...
  for(var b = 0 ; b < generalArray.length ; b++) {
    if(generalArray[b].id == href) {
      ...
      $(inscriptions).each(function() {
        var duplicateInsc = $(this).text();
        $(this).before("<p>" + duplicateInsc + "</p>");
        $(this).after( "<p class='preview'>${Preview}</p>" );
        $(this).addClass(generalArray[thisId].font);
